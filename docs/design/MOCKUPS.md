...
| **Chat (per-spark)** | http://localhost:8095/docs/design/mockups/chat.html | The per-spark chat room (phone frame). Members type emojis or react to messages to satisfy dependencies and light the spark. A **live recipe strip** under the title fills as ingredients land; a **live mini-spark** in the title reflects current state; when every dep is met a calm lighting sweep + "Spark lit" banner fire (streak +1). **Interactive** — tap a quick-emoji to post it as you, type an emoji in the input, or long-press/click a message to open the reaction sheet (reactions also satisfy deps, per spec § 4). Toggle multi-dep vs no-dep; reset cycle. |
...
- **Chat** (`chat.html`) is interactive and demonstrates the lighting mechanic end-to-end: post a dependency emoji (tap a quick-button or type it) or react to a message with one, and watch the recipe strip fill and the spark light. The quick-buttons highlight the still-needed emojis; long-press or click any message to open the reaction sheet. The dev panel toggles multi-dep vs no-dep recipes and resets the cycle. Quick-buttons and the reaction grid include a couple of non-matching emojis (🔥 ❤️) to show that only the spark's own ingredients count (spec § 4 — the spark's chat knows its own relevant emojis).
